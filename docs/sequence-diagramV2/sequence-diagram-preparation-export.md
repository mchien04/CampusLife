...
sequenceDiagram
    ...
    alt Activity không tồn tại
        ...
        alt Preparation chưa bật
            ...
            alt format == xlsx
                ES->>FG: "Tạo XSSFWorkbook với các sheet: BudgetVsActual, CashFlow, CashTransactions, FundAdvanceDebts"
                FG-->>ES:' "byte[] (workbook bytes)"'
            else format == pdf
                ES->>FG: "Tạo Document (OpenPDF / iText) với các section: Budget vs Actual, Cash Flow Summary, Invoice Status, Cash Transactions, Fund Advance Debts"
                FG-->>ES:' "byte[] (pdf bytes)"'
            end

            ES->>ES: "Tạo ExportFile(filename, contentType, bytes) filename = preparation_financial_activity_{id}_{timestamp}.{ext}"
            ES-->>EC:' "ExportFile"'

            EC->>EC: "fileResponse(file) Set Content-Disposition: attachment; filename=... Set Content-Type: application/pdf hoặc application/vnd.openxmlformats..."
            EC-->>Client: '200 OK Body: byte[] (file download)'
