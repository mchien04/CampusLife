...
public interface ActivityTaskRepository extends JpaRepository<ActivityTask, Long> {
    ...
    List<ActivityTask> findByDeadlineBetween(@Param("start") LocalDate start, @Param("end") LocalDate end);

    /**
     * Lấy danh sách nhiệm vụ có deadline trước ngày chỉ định
     */
    @Query("SELECT at FROM ActivityTask at WHERE at.deadline < :date AND at.activity.isDeleted = false")
    List<ActivityTask> findByDeadlineBeforeAndActivityIsDeletedFalse(@Param("date") LocalDate date);
    ...
    List<ActivityTask> findUpcomingDeadlines(@Param("today") LocalDate today, @Param("nextWeek") LocalDate nextWeek);
